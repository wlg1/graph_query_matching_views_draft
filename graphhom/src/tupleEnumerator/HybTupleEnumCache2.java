...
public class HybTupleEnumCache2 {
	...
	private NeighbourCache[] nbCache;

	public HybTupleEnumCache2(Query qry, ArrayList<Pool> pl) {
	   ...
	   getPlan();
		...
		nbCache = new NeighbourCache[query.V];
		for (int i = 0; i < query.V; i++) {

			nbCache[i] = new NeighbourCache();
		}
		tt = new TimeTracker();
	}

	public double enumTuples(QueryEvalStat stat) throws LimitExceededException {
	...
	private RoaringBitmap getCandBits(int cur_vertex) {
		...
		int k = hashCode(cur_vertex, num);

		bits = nbCache[cur_vertex].getValue(k);
		if (bits != null) {
			hits++;
			System.out.println("curr match: " + match[0] + " " + match[1]);
			System.out.println("hash code: " + k);
			
			System.out.println("last match: " + nbCache[cur_vertex].getStrValue(k));
			return bits;
		}
		bits = new RoaringBitmap();
		...
		nbCache[cur_vertex].insert(k, bits);

		nbCache[cur_vertex].insert(k, this.hashStr(cur_vertex, num));
		return bits;
	}

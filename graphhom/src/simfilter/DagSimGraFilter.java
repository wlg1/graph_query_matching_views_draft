...
public class DagSimGraFilter {
	...
	int[][] posList;
	public DagSimGraFilter(Query query, GraphNode[] graNodes, ArrayList<ArrayList<GraphNode>> invLstsByID,
	...
	private void pruneOneStepBUP_c(ArrayList<QNode> qnodes_c, RoaringBitmap candBits,
			boolean[] changed) {
	...
	private void clearAdjMap(ArrayList<GraphNode> srcList, QNode srcNode, DirType dir) {
		...
		for (GraphNode n : srcList) {
			AdjMap adjMap = adjMapList[posList[srcNode.id][n.id]];
			for (int i : tarList) {
				...
				for (int id : vals) {
					...
					AdjMap adjMap_c = adjMapList_c[posList[i][cn.id]];
	...
	private void pruneOneStepBUP_d(QNode parent, ArrayList<QNode> qnodes_d, ArrayList<GraphNode> candList_p,
		...
		for (GraphNode gn : candList_p) {
			AdjMap adjmap_p = adjMapList_p[posList[parent.id][gn.id]];
			for (QNode child : qnodes_d) {
				...
				for (GraphNode ni : mli.elist()) {
					...
					if (gn.id == ni.id)
						continue;
					AdjMap adjmap_c = adjMapList_c[posList[child.id][ni.id]];
	...
	private void pruneOneStepTDW_d(QNode child, ArrayList<QNode> qnodes_d, ArrayList<GraphNode> candList_c,
		...
		for (GraphNode gn : candList_c) {
			AdjMap adjmap_c = adjMapList_c[posList[child.id][gn.id]];
			for (QNode parent : qnodes_d) {
				...
				for (GraphNode par : mli.elist()) {
					...
					if (gn.id == par.id)
						continue;
					AdjMap adjmap_p = adjMapList_p[posList[parent.id][par.id]];

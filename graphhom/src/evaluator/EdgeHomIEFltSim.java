...
import simfilter.CycSimGraFilter;
import simfilter.EdgeSimGraFilter;
...
public class EdgeHomIEFltSim {
	...
	public boolean run(QueryEvalStat stat) throws LimitExceededException {
		...
		if (simfilter) {
			...
			if (mQuery.hasCycle) {
				CycSimGraFilter filter = new CycSimGraFilter(mQuery, mGraNodes, mInvLstsByID, mBitsByIDArr, true);
				filter.prune();
				mCandBitsArr = filter.getCandBits();
			} else {
				EdgeSimGraFilter filter = new EdgeSimGraFilter(mQuery, mGraNodes, mInvLstsByID, mBitsByIDArr, true);
				filter.prune();
				mCandBitsArr = filter.getCandBits();
			}

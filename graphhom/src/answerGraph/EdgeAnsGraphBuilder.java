...
public class EdgeAnsGraphBuilder {
	...
	private boolean linkOneStep(PoolEntry r, RoaringBitmap bits, ArrayList<PoolEntry> list) {

		GraphNode s = r.getValue();

		RoaringBitmap rs_and = RoaringBitmap.and(s.adj_bits_id_o, bits);

		if (rs_and.isEmpty())
			return false;

		for (int ti : rs_and) {
			PoolEntry e = list.get(bits.rank(ti) - 1);
			r.addChild(e);
			e.addParent(r);
		}

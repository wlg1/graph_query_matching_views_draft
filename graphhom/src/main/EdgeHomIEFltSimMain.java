...
import evaluator.EdgeHomIEFltSim;
...
public class EdgeHomIEFltSimMain {
	...
	public EdgeHomIEFltSimMain(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_" + fn + "_IEfltsim_Edge" + suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "GraEval_IEfltsim_Edge");
	...
	private void evaluate() {
		...
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				EdgeHomIEFltSim eva = new EdgeHomIEFltSim(query, graNodes, fb);
	...
	public static void main(String[] args) {
		...
		EdgeHomIEFltSimMain demain = new EdgeHomIEFltSimMain(dataFileN, queryFileN);

...
public class DagHomSimTJMain {
	...
	public DagHomSimTJMain(String dataFN, String queryFN) {

		queryFileN = Consts.INDIR + queryFN;
		dataFileN = Consts.INDIR + dataFN;
		String suffix = ".csv";
		String fn = queryFN.substring(0, queryFN.lastIndexOf('.'));
		outFileN = Consts.OUTDIR + "sum_" + fn + "Dag_SimTJ" + suffix;
		stats = new QueryEvalStats(dataFileN, queryFileN, "DagEval_SimTJ");
	...
	private void evaluate() {
		...
		for (int i = 0; i < Flags.REPEATS; i++) {
			for (int Q = 0; Q < queries.size(); Q++) {
				...
				DagHomSimTJ eva = new DagHomSimTJ(query, invLstsByID, bitsByIDArr, fb, bfl);
	...
	public static void main(String[] args) {
		...
		DagHomSimTJMain demain = new DagHomSimTJMain(dataFileN, queryFileN);

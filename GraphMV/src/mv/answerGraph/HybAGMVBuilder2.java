...
public class HybAGMVBuilder2 {
	...
	public HybAGMVBuilder2(Query query, GraphNode[] graNodes, BFLIndex bfl, ArrayList<MatArray> candLsts,
			ArrayList<RoaringBitmap> bitsByIDArr, ArrayList<Query> views, ArrayList<ArrayList<Pool>> vpls,
			ArrayList<CoverResult> cvs) {
	...
	private void init() {
		...
		mCoverViewNodes = new ArrayList<HashMap<Integer, HashSet<Integer>>>(mQuery.V);
		mViewNodePools = new ArrayList<HashMap<Integer, ArrayList<Pool>>>(mQuery.V);
		mEdgeCoverBits = ViewManager.getCoveredEdges(mQuery, mCovers);
		for (int i = 0; i < mQuery.V; i++) {
			HashMap<Integer, HashSet<Integer>> cvns = this.getCoverViewNodes(i);
			HashMap<Integer, ArrayList<Pool>> vnpls = this.getViewNodePools(i);
			mCoverViewNodes.add(cvns);
			mViewNodePools.add(vnpls);
		}
	...
	private HashMap<Integer, HashSet<Integer>> getCoverViewNodes(int qid) {
	...
	private HashMap<Integer, ArrayList<Pool>> getViewNodePools(int qid) {
		...
		for (CoverResult cover : mCovers) {
			...
			ArrayList<Pool> vpls = mViewPools.get(vid);
			HashSet<Integer> vns = cover.getVNodeList(qid);

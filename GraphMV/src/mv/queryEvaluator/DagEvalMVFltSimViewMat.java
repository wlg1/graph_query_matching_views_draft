...
public class DagEvalMVFltSimViewMat {
	...
	public DagEvalMVFltSimViewMat(Query query, ArrayList<Query> views, Digraph g, BFLIndex bfl, 
			ArrayList<ArrayList<Pool>> viewPools) {
	...
	public boolean run(QueryEvalStat stat) throws LimitExceededException {
		...
		for (HashMap<Integer, HashSet<Integer>> vNodeHM : mCoverViewNodes) {
			if ( vNodeHM.isEmpty() ) {
				runFilter = true;
			}
		}

...
public class HybTupleEnumer {
	...
	private void transition(int max_depth, int depth) throws LimitExceededException {
		...
		Pool pl = pools.get(cur_vertex);
		
		ArrayList<PoolEntry> elist = pl.elist();
		RoaringBitmap bitsByID = pl.getIDBits();
		...
		while (it.hasNext()) {
			...
			for(int i = 0; i<batch; ++i) {
				...
				PoolEntry e = elist.get(bitsByID.rank(buffer[i])-1);
				match[cur_vertex] = e;
	...
	private RoaringBitmap getCandBits(int cur_vertex) {
		...
		if (num == 0) {
			...
			for (PoolEntry e : pools.get(cur_vertex).elist()) {
				//bits.add(e.getPos());
				bits.add(e.getValue().id);
	...
	private int[] getPlan() {
		...
		if (Flags.ORDER == Consts.OrderType.RI)
			...
			for (int i = 0; i < query.V; i++) {
				candidates_count[i] = pools.get(i).elist().size();
			}

...
public class SimFilterBas {
	...
	public SimFilterBas(Query query, GraphNode[] graNodes, ArrayList<ArrayList<GraphNode>> invLstsByID,
	...
	private boolean backwardCheck(QEdge e) {
		...
		RoaringBitmap candBits_c = mCandBitsArr[to];
		...
		for (GraphNode gn : candList_p) {
			...
			if (axis == AxisType.child) {
				...
				found = checkChildMatch(gn, candBits_c);
			} else {
				MatArray mli = mCandLists.get(child.id);
				for (GraphNode ni : mli.elist()) {
	...
	private boolean checkChildMatch(GraphNode s, RoaringBitmap candBits) {

		if (s.N_O_SZ == 0)
			return false;

		RoaringBitmap s_bits = s.adj_bits_id_o;

		return RoaringBitmap.andCardinality(s_bits, candBits) > 0 ? true : false;

	}

	private boolean checkParMatch(GraphNode s, RoaringBitmap candBits) {

		if (s.N_I_SZ == 0)
			return false;

		RoaringBitmap s_bits = s.adj_bits_id_i;
	...
	private boolean forwardCheck(QEdge e) {
		...
		for (GraphNode gn : candList_c) {
			...
			if (axis == AxisType.child) {
				...
				found = checkParMatch(gn, candBits_p);
			} else {

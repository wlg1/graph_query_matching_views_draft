...
public class GraSimFilterNaive {
	...
	public GraSimFilterNaive(Query query, GraphNode[] graNodes, ArrayList<ArrayList<GraphNode>> invLstsByID,
	...
	private boolean backwardCheck(QEdge e) {
		...
		if (axis == AxisType.child) {
			...
			filterBwd_d(parent, child, mli.elist(), candBits);
	...
	private boolean forwardCheck(QEdge e) {
		...
		if (axis == AxisType.child) {
			...
			filterFwd_d(parent, child, mli.elist(), candBits);

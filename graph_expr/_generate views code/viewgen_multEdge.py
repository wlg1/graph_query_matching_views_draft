...
for querynum in range(num_queries):
    ...
    if num_queries > 1:
        outFN = output_name = output_prefix + '_q' + str(querynum+6)
    else:
        outFN = output_prefix
    ...
    out_file = open(output_path + ".vw", "w")
    for q, qry in enumerate(views):
        if max_num_Vedges > 1:
            ...
            plt.savefig(output_path+"_v"+ str(q) + ".jpg")
    ...
    out_file = open(output_path + "_edgeInfo.txt", "w")
    ...
    out_file = open(output_path + ".qry", "w")

...
#tunable params
numqrys = 1
# nodeSizes = [60, 80, 100, 120, 140, 160]  #per K (10000)
nodeSizes = [100, 120, 140, 160]
dataGraphType = 'Email_lb20'
# queryType = 'lb20_acyc_m_q5_2Eviews'
queryType_prefix = 'lb20_cyc_m_2Eviews_q'

# Email_lb20_100K_nodes_lb20_acyc_m_q4_2Eviews

def run(dataGraphType, queryType, nodeSizes, numqrys):
    # algoNames = ['FLTSIM', 'FLT', 'SIM']
    algoNames = ['SIM']
    ...
    for numNodes in nodeSizes: #loop thru x-axis. each x-axis tick is in diff folder
        ...
        output_prefix = 'D:/Documents/_prog/prog_cust/graphHomResults/' + dataGraphType+ '_' \
        ...
        for filename in os.listdir(input_path):
            ...
            for row in reader:
                if row[0] in algoNames:
                    ...
                    algo_totTimes[algoNames.index(row[0])][0] = float(row[3])
    ...
    plt.title('#DGnodes vs _filtTimes (secs)')
...
for q in range(6,19):
    ...
    run(dataGraphType, queryType, nodeSizes, numqrys)